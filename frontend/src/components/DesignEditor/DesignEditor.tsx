...
export function DesignEditor() {
  ...
  const selectedObject = currentPage.objects.find((obj: { id: string | null; }) => obj.id === selectedId);

  // Save to history
  const saveToHistory = (newPages: DesignPage[], newCurrentPageId: string, newSelectedId: string | null) => {
    const newHistory = history.slice(0, historyIndex + 1);
    newHistory.push({ pages: newPages, currentPageId: newCurrentPageId, selectedId: newSelectedId });
  ...
  const updateObject = (id: string, updates: Partial<DesignObject>) => {
    const newPages = pages.map(p => 
      p.id === currentPageId 
        ? { ...p, objects: p.objects.map((obj: { id: string; }) => obj.id === id ? { ...obj, ...updates } : obj) }
        : p
    );
    setPages(newPages);
    saveToHistory(newPages, currentPageId, selectedId);
  ...
  const toggleLock = (id: string) => {
    const newPages = pages.map(p =>
      p.id === currentPageId
        ? { ...p, objects: p.objects.map((obj: { id: string; locked: any; }) => obj.id === id ? { ...obj, locked: !obj.locked } : obj) }
        : p
    );
    setPages(newPages);
    saveToHistory(newPages, currentPageId, selectedId);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const pdf = new jsPDF({
        orientation: currentPage.canvasSize.width > currentPage.canvasSize.height ? 'landscape' : 'portrait',
        unit: 'px',
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        page.objects.forEach((obj: { x: any; y: any; width: any; height: any; rotation: any; opacity: any; blur: any; type: string; fontSize: any; fontFamily: string; fontWeight: any; textAlign: string; color: string; text: string; fill: string; strokeWidth: any; stroke: any; shape: string; cornerRadius: any; backgroundImage: any; backgroundPosition: { x: any; y: any; }; backgroundScale: any; imageUrl: string; }) => {
          ...
          if (obj.type === 'text') {
            ...
            objElement.style.border = `${obj.strokeWidth || 0}px solid ${obj.stroke || '#000000'}`;
            ...
            if (obj.backgroundImage) {
              ...
              objElement.style.backgroundPosition = `${obj.backgroundPosition?.x || 50}% ${obj.backgroundPosition?.y || 50}%`;
              objElement.style.backgroundSize = `${(obj.backgroundScale || 1) * 100}%`;
        ...
        if (i > 0) {
          pdf.addPage([page.canvasSize.width, page.canvasSize.height], 
            page.canvasSize.width > page.canvasSize.height ? 'landscape' : 'portrait');
        }
        
        pdf.addImage(imgData, 'PNG', 0, 0, page.canvasSize.width, page.canvasSize.height);
      ...
      <Toolbar 
        ...
        onClose={() => {
          if (confirm('Are you sure you want to close? Any unsaved changes will be lost.')) {
            ...
            setSelectedId(null);
            setHistory([{ pages: [], currentPageId: 'page-1', selectedId: null }]);
            setHistoryIndex(0);
          }
        }}
      />
