...
import { Eye, EyeOff, Copy, Trash2, GripVertical, Lock, Unlock } from 'lucide-react';
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs md:text-sm truncate">{getObjectName(obj)}</div>
                      <div className="text-xs opacity-60 hidden md:block">{getObjectIcon(obj)}</div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className={`p-1 hover:bg-gray-200 rounded ${obj.locked ? 'text-orange-500' : ''}`}
                        title={obj.locked ? "Unlock" : "Lock"}
                      >
                        {obj.locked ? <Lock className="w-3 h-3 md:w-4 md:h-4" /> : <Unlock className="w-3 h-3 md:w-4 md:h-4" />}
                      </button>
